...
def write_html(dic,week):
    File = open(r'C:\Users\mi\Desktop\复变函数题库\第'+str(week)+'周.html','w',encoding='utf-8')
    head = '''<html>
    <table border="1">
...
for week in range(weeks):
    ...
    for i in range(10):
        try:
            ...
            if driver.find_element_by_class_name('ux-modal_bd') == None:
                key == False
            else:
                key = True

            while key == True:
                button = driver.find_element_by_class_name('ux-modal-btn')
                button.click()
                time.sleep(1)
                para0 += 1
                url = url[:-10] + str(para0)
